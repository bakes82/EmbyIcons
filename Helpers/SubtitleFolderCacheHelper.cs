...
{
    private static readonly ConcurrentDictionary<string, List<string>> _folderSubtitleCache =
        new ConcurrentDictionary<string, List<string>>();

    public static List<string> GetSubtitleFilesForFolder(string folderPath, string[] subtitleExtensions)
    {
        if (_folderSubtitleCache.TryGetValue(folderPath, out var cached)) return cached;
