...
internal static class FileUtils
{
    public static async Task SafeCopyAsync(string source, string dest)
    {
        ...
        using (var sourceStream =
               new FileStream(source, FileMode.Open, FileAccess.Read, FileShare.Read, bufferSize, true))
        {
            using (var destStream =
                   new FileStream(dest, FileMode.Create, FileAccess.Write, FileShare.None, bufferSize, true))
            {
    ...
    public static HashSet<string> GetAllowedLibraryIds(ILibraryManager libraryManager, string? selectedLibrariesCsv)
    {
        ...
        if (string.IsNullOrWhiteSpace(selectedLibrariesCsv)) return allowedIds; // no restriction
        ...
        foreach (var lib in libraryManager.GetVirtualFolders())
            if (lib.Locations != null)
                foreach (var loc in lib.Locations)
                {
                    ...
                    if (!string.IsNullOrEmpty(normLoc) && normItemPath.StartsWith(normLoc)) return lib.Id;
                }
