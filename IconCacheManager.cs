...
{
    private readonly ConcurrentDictionary<string, CachedIcon> _audioIconImageCache =
        new ConcurrentDictionary<string, CachedIcon>();

    private readonly ConcurrentDictionary<string, string?> _audioIconPathCache =
        new ConcurrentDictionary<string, string?>();

    private readonly TimeSpan _cacheTtl;

    // Track file last write times to detect changes
    private readonly ConcurrentDictionary<string, DateTime> _iconFileLastWriteTimes =
        new ConcurrentDictionary<string, DateTime>();

    // Max degree of parallelism for icon loading to avoid overloading CPU/IO
    private readonly int _maxParallelism;

    private readonly ConcurrentDictionary<string, CachedIcon> _subtitleIconImageCache =
        new ConcurrentDictionary<string, CachedIcon>();

    private readonly ConcurrentDictionary<string, string?> _subtitleIconPathCache =
        new ConcurrentDictionary<string, string?>();
    ...
    {
        if (_iconsFolder == null || !Directory.Exists(_iconsFolder)) return;
        ...
        if (!anyChanged && DateTime.UtcNow - _lastCacheRefreshTime <= _cacheTtl) return;

        _iconFileLastWriteTimes.Clear();
        foreach (var kvp in currentWriteTimes) _iconFileLastWriteTimes[kvp.Key] = kvp.Value;
        ...
        await Task.Run(() =>
                       {
                           ...
                           Parallel.ForEach(_audioIconPathCache.Values.Where(f => f != null)!, po, path =>
                                            {
                                                if (!string.IsNullOrEmpty(path)) TryLoadAndCacheIcon(path!, false);
                                            });

                           Parallel.ForEach(_subtitleIconPathCache.Values.Where(f => f != null)!, po, path =>
                                            {
                                                if (!string.IsNullOrEmpty(path)) TryLoadAndCacheIcon(path!, true);
                                            });
                       }, cancellationToken);
        ...
        var versionChanged = version != _currentIconVersion;
        _currentIconVersion = version;

        if (anyChanged && versionChanged) CacheRefreshedWithVersion?.Invoke(this, _currentIconVersion);
    }
    ...
    private void CacheIcon(string iconPath, SKImage? image, DateTime lastWrite, bool isSubtitle)
    {
        ...
        cache.AddOrUpdate(iconPath, cacheEntry, (key, old) =>
                                                {
                                                    old.Image?.Dispose();
                                                    return cacheEntry;
                                                });
    ...
    public SKImage? GetCachedIcon(string langCodeKey, bool isSubtitle)
    {
        ...
        var iconPath = ResolveIconPathWithFallback(langCodeKey, _iconsFolder, pathCache);

        if (iconPath == null || !File.Exists(iconPath)) return null;
    ...
    private string? ResolveIconPathWithFallback(string langCodeKey,
                                                string folderPath,
                                                ConcurrentDictionary<string, string?> cache)
    {
        langCodeKey = langCodeKey.ToLowerInvariant();

        if (cache.TryGetValue(langCodeKey, out var path) && !string.IsNullOrEmpty(path)) return path;
...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    private static string ComputeIconFilesVersion(string[] files)
    {
        ...
        var combined = string.Join("|", files.OrderBy(f => f)
    ...
    private sealed class CachedIcon
    {
        ...
        {
            return DateTime.UtcNow - _loadTimeUtc > ttl;
        }
    }
}